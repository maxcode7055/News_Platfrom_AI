...
#     tokenizer.tgt_lang = "fa_IR"  # Farsi (Persian)
#     encoded_english_text = tokenizer(text, return_tensors="pt", padding=True, truncation=True)
#     generated_tokens = model.generate(**encoded_english_text, forced_bos_token_id=tokenizer.lang_code_to_id["fa_IR"])
#     out = tokenizer.batch_decode(generated_tokens, skip_special_tokens=True)
#     return out
...
# sentences = """Learn the basics and advanced concepts of natural language processing (NLP) with our complete NLP tutorial and get ready to explore the vast and exciting field of NLP, where technology meets human language.
...
# translated_text = translate_text(sentences, model, tokenizer)
